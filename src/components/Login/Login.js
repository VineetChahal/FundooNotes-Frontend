...
export default function Login() {
    ...
    const handleLogin = () => {
        ...
        .then((response) => {
            ...
            const tokenObject = response.data.token; // The whole token object
            if (!tokenObject.token) {
                throw new Error("No access token received from backend");
            }
            localStorage.setItem("token", JSON.stringify(tokenObject)); // Store as JSON string
            console.log("Token object stored:", tokenObject);
